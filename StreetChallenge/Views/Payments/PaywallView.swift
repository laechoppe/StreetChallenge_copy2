...
struct PaywallView: View {
    ...
    private func restorePurchases() {
        ...
        Purchases.shared.restorePurchases { customerInfo, error in
            ...
            if let error = error {
                self.message = "Restore failed: \(error.localizedDescription)"
                return
            }

            if let customerInfo = customerInfo, !customerInfo.entitlements.active.isEmpty {
                self.message = "Purchases successfully restored!"
            } else {
                self.message = "No purchases to restore."
            }
        }
    }
}
...
private struct PaywallContent: View {
    ...
    var body: some View {
        NavigationView {
            ZStack {
                ...
                List {
                    Section(header: Text("\nStreet Challenge Premium"), footer: Text(Self.footerText)) {
    ...
    private static let footerText = Constants.paymentTerms
...
#Preview {
    let offeringIdentifier = "offering"

    let product1 = TestStoreProduct(
